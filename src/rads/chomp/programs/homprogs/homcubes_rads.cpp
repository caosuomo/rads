...
int Xspacedim = (Xcubes. size ()) ? Xcubes [0]. dim () : -1;
	...
	// forget the coordinates of cubes if they are not needed
	// anymore -- JJB: Tried to comment out this section with
	// CHATTER macro. But output was funky, eg.
	//  {          0   <--- extra spaces and 0 added for not apparent reason(??)
	...
	savebetti (bettiname, mapname ? hom_cgraph : hom_cx,
        ...
        // JJB -- 7/9/2013. defined simplified macro near top
	#ifdef SIMPLIFIED
	// show the Betti numbers for X in the simplified version
	{
		...
		sout << '\n';
                    ...
                    // JJB -- 7/9/13
                    // if uncommented, this turns off all other output
                    //below.
                    //sout. show = false;
	}
	#endif
	...
	if (mapname && indexmap)
	{
		...
		if (invertible)
		{
			...
			// sout << "\n";
                        ...
                        // This prints out the Pythonic string for
                        // conversion to a dictionary representing the
                        // index map on all (relavant) dimensions.
			sout << "\n{";
			hcomp. show(sout, "", "", "", level);
			sout << "}\n";
