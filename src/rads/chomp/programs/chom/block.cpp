...
/*! \file block.cc
    ...
    This file contains methods for block class including
    routines for creating a new block of one d-cell
    and all its boundary cells.
...
{
   bitcode dl(cube_bits+DIM); // minimum vertex -- diagonal
   dl.SetAs(this->bc);

   bitcode du(cube_bits+DIM); // maximum vertex -- diagonal
   du.SetAs(this->bc);
   for(int i=0; i<DIM; ++i)
     du.Increment(i);
	
   cell* new_cell=NULL;
   Create(&new_cell,dl,du,DIM,c,bdry_verts);
   new_cell->MarkInterior();

//   for(int i=0; i<DIM+1; ++i)
//     assert(cells[i].Empty());
   cells[DIM].Insert(new_cell);
}
...
//	     cout << "Ncell: " << f << endl;
	     ...
	     while(bit!=bdry->End())
		  ...
		  ++bit;
             ...
             if (gen_flag[dim])
	       {
                  f->diagonal[0]=new bitcode(cube_bits+DIM);
	          f->diagonal[1]=new bitcode(cube_bits+DIM);
...
//   assert((DIM>2)&&(DIM==dim));
   
   ncell* new_cell=new ncell(DIM);
