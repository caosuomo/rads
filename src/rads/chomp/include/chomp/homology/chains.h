...
// Started in 1997. Last revision: April 4, 2011.
...
/* JJB -- 3/22/13 */
/* Keep label input for simplicity with function definition.  */
template <class euclidom>
inline outputstream &chain<euclidom>::show (outputstream &out,
	const char *label) const
{
	...
	const char *maplabel, const char *xlabel, const char *ylabel) const
{
	...
	for (int i = 0; i < ncols; ++ i)
	{
	        // open dict to represent map on generator i
	  	out << "{";
		// write domain generator
		out << (i + 1) << ": [";
		// show image of index map of generator i
		cols [i]. show(out, ylabel) << "]";
		out << "}, ";  // close dict map
	}
	return out;
	...
	const int *level) const
{
	for (int q = 0; q < len; ++ q)
	{
		...
		// JJB -- 3/24/13
		// out << "# " << q << "\n";
		// list of maps, one for each generator
		out << q << " : [ ";
