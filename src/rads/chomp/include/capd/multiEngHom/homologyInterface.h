...
void makeSet(P_CubSet const& set, chomp::homology::SetOfCubes& Xcubes){
...
template <typename P_CubSet>
void makeMVInclusionMap(P_CubSet const& set, chomp::homology::CombinatorialMultivaluedMap& Fcubmap){
  using namespace chomp::homology;
  for(typename P_CubSet::PointCoordIterator it=set.begin();it<set.end();++it)
  if(it.getBit()){
    int dim=set.embDim();
...
int getHomology(
    ...
    chomp::homology::Chain *&hom,            // pseudo chain with computted Betti numbers (?)
    chomp::homology::Chain **&gen,           // reference to table of table of generators
    chomp::homology::CubicalComplex *&gcompl // reference to table of geometric form of generators
){
  using namespace chomp::homology;
  SetOfCubes Xcubes;
  makeSet(X,Xcubes);
...
template <typename P_CubSet>
CRef<std::vector<chomp::vectalg::Matrix<int,0,0> > >
getHomologyInclusionMap(
   ...
   chomp::homology::Chain *&hom_cx, chomp::homology::Chain *&hom_cy,
   chomp::homology::Chain **&gen_cx, chomp::homology::Chain **&gen_cy,
   chomp::homology::CubicalComplex *&gcompl_cx,chomp::homology::CubicalComplex *&gcompl_cy
){
  using namespace chomp::homology;
...
//  bool generators=false;
  ...
  CRef<std::vector<chomp::vectalg::Matrix<int,0,0> > > mCR( new std::vector<chomp::vectalg::Matrix<int,0,0> >() );
  for(int q=0;q<=maxlevel_cx;++q){
    int nRows=(*hom_f)[q].getnrows();
    int nCols=(*hom_f)[q].getncols();
    mCR().push_back(chomp::vectalg::Matrix<int,0,0>(nRows,nCols));
...
inline int extractBetti(const chomp::homology::chain<euclidom> &c){
  using namespace chomp::homology;
  int countfree = 0;
...
template <class euclidom>
string groupToString(const chomp::homology::chain<euclidom> &c){
  using namespace chomp::homology;
...
template <class euclidom>
string homologyToString(const chomp::homology::chain<euclidom> *hom, int maxlevel){
  using namespace chomp::homology;
  if (!hom) return "";
  ostringstream result;
...
std::string homologyViaHomologyPackage(const P_CubSet& A_cubSet){
  chomp::homology::Chain *hom;            // pseudo chain with computted Betti numbers
  chomp::homology::Chain **gen=0;         // reference to table of table of generators
  chomp::homology::CubicalComplex *gcompl=0;
...
std::string inclHomologyViaHomologyPackage(CRef<P_CubSet> A_subSetCR,CRef<P_CubSet> A_supSetCR){
  ...
  int maxlevel_cx;
  int maxlevel_cy;
  chomp::homology::Chain *hom_cx;
  chomp::homology::Chain *hom_cy;
  chomp::homology::Chain **gen_cx=0;
  chomp::homology::Chain **gen_cy=0;
  chomp::homology::CubicalComplex *gcompl_cx=0;
  chomp::homology::CubicalComplex *gcompl_cy=0;
  CRef<std::vector<chomp::vectalg::Matrix<int,0,0> > > mCR=getHomologyInclusionMap(
    A_subSetCR(),
    empty,
    A_supSetCR(),
    empty,
